...
represent real problems with Bisque, but many are problems in
libraries that Bisque depends on, and many of the test cases
...
from bisque import Bisque, ParserRejectedMarkup
...
class TestFuzz:
    ...
    def fuzz_test_with_css(self, filename):
        ...
        try:
            soup = Bisque(data[1:], features=parsers[idx])
    ...
    # to a string. This is an issue with Bisque which was fixed
    ...
    def test_deeply_nested_document_without_css(self, filename):
        ...
        # been fixed.
        markup = self.__markup(filename)
        Bisque(markup, "html.parser").encode()
    ...
    def test_html5lib_parse_errors_without_css(self, filename):
        markup = self.__markup(filename)
        print(Bisque(markup, "html5lib").encode())
